...
### 2026-09-23 · 游标交互统一（图表改拖动制）+ 悬浮倍率降级为折叠项
...
- **悬浮窗倍率从常驻控件降级为「校准」折叠项**：底栏曾同屏出现两个都叫「速度/倍速」但语义
  完全无关的控件 —— 悬浮区的「游戏速度」（悬浮窗时钟倍率）与语音区的「倍速 1×/2×/4×/8×」
  （播报时间轴推进倍率），实测让人困惑。现在常驻只留 **版式 / 底板 / 开始**，
  「回放速度」收进 `<details>校准`（默认收起；**样式全部内联，零 CSS 依赖**）。
  展开时靠 `#vb` 的 ResizeObserver → `--vbh` 自动补底部留白（见 `main.js::trackBottomBarHeight`），
  底栏向上生长、不压住内容。
