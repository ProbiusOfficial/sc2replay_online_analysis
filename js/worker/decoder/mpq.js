 ...
 */
export function hashFileName(name, type) {
...
function decryptUint32Block(data, key) {
    ...
    if (count * 4 < data.length)
        out.set(data.subarray(count * 4));
    return out;
}
// ---------------------------------------------------------------- 解压
async function inflate(compressed, hint) {
    try {
        ...
        throw new MpqError("DECOMPRESS_FAILED", `deflate 解压失败（${hint}）：${cause instanceof Error ? cause.message : String(cause)}`);
    }
}
/**
 ...
 */
async function decompressBlock(data, hint, decompressor) {
    if (data.length === 0)
        return data;
    ...
    if (compressionType === COMPRESSION_ZLIB)
        return inflate(data.subarray(1), hint);
    if (compressionType === COMPRESSION_BZIP2) {
        if (!decompressor) {
            throw new MpqError("UNSUPPORTED_COMPRESSION", `遇到 bzip2 块（${hint}）但未注入解压后端。浏览器无原生 bzip2，` +
                `请用 openMpqArchive(source, { decompressor }) 注入 wasm 侧实现。`);
        }
        return decompressor.bzip2(data.subarray(1));
    }
    throw new MpqError("UNSUPPORTED_COMPRESSION", `不支持的压缩类型 ${compressionType}（${hint}）`);
}
// ---------------------------------------------------------------- 归档
const HEADER_STRUCT_SIZE = 32;
const USER_DATA_HEADER_SIZE = 16;
const TABLE_ENTRY_SIZE = 16;
function readAsciiMagic(view, offset) {
...
export class MpqArchive {
    ...
    header;
    userDataHeader;
    constructor(bytes, header, userDataHeader, hashTable, blockTable, decompressor) {
        this.bytes = bytes;
    ...
    static async open(source, options = {}) {
        ...
        if (magic0 === MAGIC_MPQ_USER_DATA) {
            ...
            throw new MpqError("BAD_MAGIC", `不是 MPQ/SC2Replay 文件：首 4 字节为 ${[...bytes.subarray(0, 4)]
        ...
        const hashTable = archive.readTable(header.hashTableOffset, header.hashTableEntries, hashTableKey, (v, o) => ({
        ...
        const blockTable = archive.readTable(header.blockTableOffset, header.blockTableEntries, blockTableKey, (v, o) => ({
            offset: v.getUint32(o, true),
            archivedSize: v.getUint32(o + 4, true),
            size: v.getUint32(o + 8, true),
            flags: v.getUint32(o + 12, true),
        }));
        return new MpqArchive(bytes, header, userDataHeader, hashTable, blockTable, options.decompressor);
    }
    static readUserDataHeader(bytes, view) {
        if (bytes.length < USER_DATA_HEADER_SIZE)
            return null;
        if (view.getUint8(3) !== MAGIC_MPQ_USER_DATA)
            return null;
    ...
    static readHeader(bytes, view, offset) {
        if (offset + HEADER_STRUCT_SIZE > bytes.length) {
            throw new MpqError("TRUNCATED", `MPQ header 偏移 ${offset} 处不足 ${HEADER_STRUCT_SIZE} 字节`);
        }
    ...
    readTable(tableOffset, entries, key, parse) {
        if (entries === 0)
            return [];
        const absolute = tableOffset + this.header.offset;
        const length = entries * TABLE_ENTRY_SIZE;
        if (absolute + length > this.bytes.length) {
            throw new MpqError("TRUNCATED", `表读取越界：offset=${absolute} length=${length} fileSize=${this.bytes.length}`);
        }
    ...
    get hashTableEntryCount() {
        return this.hashTable.length;
    }
    /** block table 条目数（调试/对照用）。 */
    get blockTableEntryCount() {
    ...
    async readBlock(block, name) {
        if ((block.flags & MPQ_FILE_EXISTS) === 0)
            return null;
        if (block.archivedSize === 0)
            return null;
        if ((block.flags & MPQ_FILE_ENCRYPTED) !== 0) {
            throw new MpqError("ENCRYPTED_NOT_SUPPORTED", `文件 ${name} 被加密，本实现不支持（实测样本均未加密）`);
        }
        const start = block.offset + this.header.offset;
        const end = start + block.archivedSize;
        if (end > this.bytes.length) {
            throw new MpqError("TRUNCATED", `文件 ${name} 数据越界：offset=${start} size=${block.archivedSize} fileSize=${this.bytes.length}`);
        }
        ...
        if (tableBytes > raw.length) {
            throw new MpqError("TRUNCATED", `文件 ${name} 的 sector 偏移表越界：需要 ${tableBytes} 字节，实得 ${raw.length}`);
        }
        ...
        for (let i = 0; i < iterations; i += 1) {
            ...
            let sector = raw.subarray(positions[i], positions[i + 1]);
...
export function openMpqArchive(source, options = {}) {
