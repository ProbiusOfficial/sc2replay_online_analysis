...
import type { BuildOrderEntry } from "./build_order.js";
...
/** 一条星空加速行。形状逐字复刻旧链路，`unit` / `target` 恒空是**行为**而非缺失。 */
export interface RecallRow {
  /** 我方 pid（`userIdToPlayerId` 的两跳桥结果，与 `BuildOrderEntry.playerId` 同一口径）。 */
  playerId: number;
  /** 指令帧（`SCmdEvent._gameloop`）。排序用；不进旧链路形状。 */
  startFrame: number;
  /** `floor(startFrame / 16)` —— 与旧链路 `int(frame) >> 4` 一致。 */
  start_time: number;
  /** 旧链路恒为 `None`，照实复刻。 */
  supply: null;
  /** **恒为空串**（点目标，指令里没有单位名）。别填东西。 */
  unit: "";
  /** **恒为 `null`**，同上。 */
  target: null;
  is_worker: false;
...
export interface CollectRecallInput {
  /** `replay.game.events` 的解码结果（recall 是**指令**，不在 tracker 流里）。 */
   ...
   * **不要传跨版本并集** —— 并集里的 723/724 会把时空加速误判成星空加速（反之亦然）。
   */
  links: AbilityLinks;
}
 ...
 */
export function collectRecalls(input: CollectRecallInput): Map<number, RecallRow[]> {
  const { gameEvents, userToPlayer, links } = input;
  const out = new Map<number, RecallRow[]>();
  // 没有 recall link（绝大多数录像）时直接返回，省掉整趟扫描。
  if (links.recall.size === 0) return out;

  for (const event of gameEvents) {
    const abil = event.m_abil as Record<string, unknown> | undefined;
    if (!abil) continue;
    if (!links.recall.has(Number(abil.m_abilLink))) continue;
    ...
    else out.set(pid, [row]);
  }

  return out;
}

/** `build_order` 最终行：tracker 派生条目 + 指令派生的 recall 行。 */
export type BuildOrderRow = BuildOrderEntry | RecallRow;

/** 类型守卫。因 `RecallRow` 独有 `_kind` 字段，`BuildOrderEntry` 上没有它。 */
export function isRecallRow(row: BuildOrderRow): row is RecallRow {
  return (row as RecallRow)._kind === "recall";
}
 ...
 *   但比静默丢掉更安全）。
 */
export function mergeRecalls(
  byPlayer: ReadonlyMap<number, BuildOrderEntry[]>,
  recalls: ReadonlyMap<number, RecallRow[]>,
): Map<number, BuildOrderRow[]> {
  const out = new Map<number, BuildOrderRow[]>();
  const pids = new Set<number>([...byPlayer.keys(), ...recalls.keys()]);

  for (const pid of pids) {
    const base = byPlayer.get(pid) ?? [];
    const extra = [...(recalls.get(pid) ?? [])].sort((a, b) => a.startFrame - b.startFrame);
    const merged: BuildOrderRow[] = [];
    let i = 0;
