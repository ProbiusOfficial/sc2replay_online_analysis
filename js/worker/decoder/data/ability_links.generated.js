 ...
 */
export const ABILITY_LINK_RANGES = [
...
export const NO_ABILITY_LINKS = { chrono: new Set(), recall: new Set() };
/** 取**首个匹配**的区间 —— 顺序即优先级，不能改。 */
export function abilityLinksForBuild(build, expansion) {
    if (!expansion)
        return NO_ABILITY_LINKS;
    for (const range of ABILITY_LINK_RANGES) {
        if (range.expansion !== expansion)
            continue;
        if (build < range.lo)
            continue;
        if (range.hi !== null && build >= range.hi)
            continue;
