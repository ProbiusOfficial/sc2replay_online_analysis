...
import {
  BUILD_DATA_HISTORY,
  BUILD_TIMES,
  CHANGED_EXCLUDED_UNITS,
  EXCLUDED_UNITS,
  GAME_LOOPS_PER_SECOND,
  UPGRADE_EXCLUDED,
  WORKER_UNITS,
  type BuildTimeEntry,
} from "./data/build_times.generated.js";
import { applyChronoBoost, type ChronoBoosts } from "./chrono.js";
import type { DecodedEvent } from "./events.js";

/** 起点的来源。用于透明化，也是排查偏差的第一入口。 */
export type StartSource =
  /** 建筑的 `SUnitInitEvent` 帧 —— 精确。 */
  | "init"
  /** 虫族单位由 Egg 的 `SUnitTypeChangeEvent` 帧定位 —— 精确。 */
  | "egg"
  /** 由建造时长表回推 —— 受表精度限制。 */
  | "table"
  /** 表里没有这个单位，起点退化成出生时刻（旧链路会加 `(Error on build time)`）。 */
  | "fallback"
  /** 变形事件（Lair / Hive / OrbitalCommand 等）。 */
  | "morph";
...
export interface BuildOrderEntry {
  ...
  /** 起点 game loop（回推后的）。 */
  startFrame: number;
  /** 终点 game loop。建筑来自 `SUnitDoneEvent`；单位等于出生帧；拿不到时为 null。 */
  finishFrame: number | null;
  /** 起点秒数，`floor(startFrame / 16)` —— 与旧链路 `>> 4` 对齐。 */
  start_time: number;
  /** 起点时刻的补给占用。拿不到为 null。 */
  supply: number | null;
  /** 单位名。表里没有时会带上旧链路一致的 `(Error on build time)` / `(upgrade missing)` 后缀。 */
  unit: string;
  ...
  /** 触发本条的事件类型，便于回溯。 */
  fromEvent: "born" | "init" | "upgrade" | "change";
  /** 回推所用时长（game loop）。`init` / `fallback` 为 0。 */
  buildLoops: number;
  /** 起点是否被时空加速修正过（旧链路的 `is_chronoboosted`）。 */
  is_chronoboosted: boolean;
}

export interface BuildOrderOptions {
  /**
   * 虫族单位是否用 Egg→Born 的实测帧定位起点。默认 `true`（更准）。
   * 置 `false` 可退回与旧链路完全一致的「一律查表」行为。
   */
  exactZergStart?: boolean;
  /** 录像的 Unix 时间戳（秒），用于按平衡性热修回滚建造时长。缺省用当前值。 */
  replayTimestamp?: number | null;
  /**
   * 时空加速区间与倍率。不给就完全不做加速修正（= 星灵起点会偏早）。
   * 由 `collectChronoBoosts` 产出，倍率取 `chronoModel().multiplier`。
   */
  chrono?: { boosts: ChronoBoosts; multiplier: number } | null;
}

export interface BuildOrderResult {
  /** 玩家 id → 建造表（按起点帧稳定升序）。 */
  byPlayer: Map<number, BuildOrderEntry[]>;
  /** 诊断计数，验收脚本会断言。 */
  stats: {
    /** 因 `EXCLUDED_UNITS` 被丢弃的 Born 数（含中立单位）。 */
    excludedBorn: number;
    /** 因起点为 0 帧被丢弃的 tracker 事件数（开局单位 / 地图中立）。 */
    frameZeroSkipped: number;
    /** 走 Egg 精确定位的条数。 */
    eggLinked: number;
    /** 表里没有、起点退化成出生时刻的条数。 */
    fallbackTiming: number;
    /** 被 `CHANGED_EXCLUDED_UNITS` 丢弃的变形事件数。 */
    changedExcluded: number;
    /** 起点为 Init 但建造时长表里没有的建筑数（旧链路同样不回推，不影响正确性）。 */
    buildingWithoutTable: number;
    /** 被时空加速修正过起点的条数。 */
    chronoboosted: number;
  };
}
...
function unitTagKey(index: unknown, recycle: unknown): string {
 ...
 */
export function resolveBuildTimes(timestamp?: number | null): Record<string, BuildTimeEntry> {
  if (!timestamp) return BUILD_TIMES;

  let overrides: Record<string, BuildTimeEntry> | null = null;
...
class SupplySeries {
  private readonly points = new Map<number, SupplyPoint[]>();

  /** 照搬 spawningtool 的初值：`[[0, 6]]`。 */
  ensure(playerId: number): void {
    if (!this.points.has(playerId)) this.points.set(playerId, [{ frame: 0, value: 6 }]);
  }

  record(playerId: number, frame: number, value: number): void {
    this.ensure(playerId);
    this.points.get(playerId)!.push({ frame, value });
  }

  /** 旧的 LotV 早期录像修正：帧 1 的补给若是 12，回写初值。 */
  patchInitial(playerId: number, frame: number, value: number): void {
    if (frame !== 1 || value !== 12) return;
    const series = this.points.get(playerId);
    if (series && series.length === 1) series[0].value = 12;
  }

  /** 取「帧 ≤ frame 的最后一个记录」，没有则第一个，全空则 null。 */
  at(playerId: number, frame: number): number | null {
    const series = this.points.get(playerId);
...
export function extractBuildOrder(
  ...
  const eggFrames = new Map<string, number>();
  /** 建筑 tag → `SUnitInitEvent` 帧，用于配 `SUnitDoneEvent` 得终点。 */
  const initFrames = new Map<string, number>();
  /**
   * unit tag → 归属玩家。
   *
  ...
  const byPlayer = new Map<number, BuildOrderEntry[]>();
  const stats = {
    excludedBorn: 0,
    frameZeroSkipped: 0,
    eggLinked: 0,
  ...
  const push = (entry: BuildOrderEntry): void => {
  ...
  ): { startFrame: number; chronoboosted: boolean } => {
    if (!chronoBoosts) return { startFrame: frame - loops, chronoboosted: false };
    const result = applyChronoBoost(frame, loops, playerId, builtFrom, chronoBoosts, chronoMultiplier);
    if (result.chronoboosted) stats.chronoboosted += 1;
    return result;
  };
  ...
  for (const event of trackerEvents) {
    ...
    switch (kind) {
      case "SPlayerStatsEvent": {
        ...
        const raw = (event.m_stats as Record<string, unknown> | undefined)?.m_scoreValueFoodUsed;
      ...
      case "SUnitInitEvent": {
        ...
        if (!data) stats.buildingWithoutTable += 1;
        break;
      }
      ...
      case "SUpgradeEvent": {
        ...
        if (pid === 0 || UPGRADE_EXCLUDED.includes(name)) break;
        const data = table[name];
  ...
  for (const list of byPlayer.values()) list.sort((a, b) => a.startFrame - b.startFrame);
 ...
 */
export function collectTagToUnitName(trackerEvents: DecodedEvent[]): Map<string, string> {
  const out = new Map<string, string>();
  for (const event of trackerEvents) {
    const kind = event._event.slice(event._event.lastIndexOf(".") + 1);
    if (kind !== "SUnitBornEvent" && kind !== "SUnitInitEvent") continue;
    const key = unitTagKey(event.m_unitTagIndex, event.m_unitTagRecycle);
 ...
 */
export function toLegacyEntry(entry: BuildOrderEntry): {
  start_time: number;
  supply: number | null;
  unit: string;
  _kind: "unit";
