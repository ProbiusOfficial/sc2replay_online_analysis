 ...
 */
export function chronoModel(unixTimestamp, expansion, cooperative) {
 ...
 */
export function expansionFromBaseBuild(baseBuild) {
    const b = baseBuild ?? 0;
    if (b >= 38749)
        return "LotV"; // LotV 上线（3.0）
    if (b >= 26414)
        return "HotS"; // HotS 上线（2.0.4）
    return "WoL";
}
/**
 ...
 */
function cacheHandleHash(handle) {
    if (!(handle instanceof Uint8Array) || handle.length < 40)
        return null;
    let hex = "";
    for (let i = 8; i < 40; i += 1)
        hex += handle[i].toString(16).padStart(2, "0");
    return hex;
}
/**
 ...
 */
export function expansionFromDetails(details) {
 ...
 */
export function unixTimestampFromDetails(details) {
    const raw = readPath(details, ["m_timeUTC"]);
    if (raw === undefined || raw === null)
        return 0;
    if (typeof raw === "bigint")
        return Number((raw - 116444735995904000n) / 10000000n);
 ...
 */
export function userIdToPlayerId(initData, details) {
    const out = new Map();
...
export function collectChronoBoosts(input) {
    ...
    for (const event of gameEvents) {
        ...
        if (!abil)
            continue;
        if (!links.chrono.has(Number(abil.m_abilLink)))
            continue;
        if (Number(abil.m_abilCmdIndex) !== 0)
            continue;
        const target = event.m_data?.TargetUnit;
        if (!target || target.m_tag === undefined || target.m_tag === null)
            continue;
 ...
 */
function buildWindowedBoosts(commands, durationLoops) {
    const out = new Map();
    for (const [pid, list] of commands) {
        ...
        for (const [building, frames] of byBuilding) {
            ...
            for (const frame of frames) {
                ...
                else if (current[1] > frame)
                    current[1] = frame + durationLoops;
                else {
                    merged.push(current);
                    current = [frame, frame + durationLoops];
                }
 ...
 * 「如果玩家从不移动加速目标，结果可能不准」。
 */
function buildLotvBoosts(commands, totalFrames) {
    const out = new Map();
    for (const [pid, list] of commands) {
        const ranges = {};
        let lastBuilding = "Nexus";
        let lastStartFrame = 0;
        const timeline = [...list, ["", totalFrames]];
        for (const [building, frame] of timeline) {
            const arr = ranges[lastBuilding];
            if (arr)
                arr.push([lastStartFrame, frame]);
            else
                ranges[lastBuilding] = [[lastStartFrame, frame]];
 ...
 */
export function applyChronoBoost(frame, buildLoops, playerId, builtFrom, boosts, multiplier) {
