...
export interface ProtocolSelection {
  ...
  /** 这份协议表对应的官方版本号（`compute.d.ts` 里也叫 build）。 */
  protocolBuild: number;
  /**
   * 是否走了降级。
   *
   * `true` 表示该录像所属补丁的官方定义**没有内置**，用的是替代版本，
   * 解析结果可能缺字段或错位。上层应据此提示用户。
   */
  degraded: boolean;
  /** 面向排查与 UI 的说明文字。 */
  note: string;
...
function officialBound(build: number): number | null {
  let low = 0;
...
function nearestAvailable(build: number): number {
 ...
 */
export function selectProtocolTables(baseBuild: number): ProtocolSelection {
  ...
  return {
    ...
    degraded: true,
    note:
      `baseBuild ${baseBuild} ${boundText}，该补丁的官方定义未内置；` +
 ...
 */
export function probeBaseBuild(headerBytes: Uint8Array): {
  baseBuild: number;
  probeBuild: number;
} {
  ...
  for (const build of candidates) {
    ...
    try {
      ...
      const header = decoder.instance(tables.TYPEIDS.replayHeader) as
        | { m_version?: { m_baseBuild?: unknown } }
        | null;
  ...
  throw new Error(
    `所有内置协议都读不出 replay.header 的 baseBuild：${String(lastError)}`,
  );
}
