...
import { ERROR_CODES, type WorkerRequest, type WorkerResponse } from "./contract.js";
 ...
 */
let decompressor: ReturnType<typeof createWasmDecompressor> | null = null;

function getDecompressor(): ReturnType<typeof createWasmDecompressor> {
  if (!decompressor) decompressor = createWasmDecompressor();
  return decompressor;
}

function fail(id: string, md5: string, message: string, code: string): void {
...
workerScope.addEventListener("message", async (event) => {
  ...
  const pending = request as Partial<{ id: string; md5: string }>;
  fail(
    pending.id ?? "",
    pending.md5 ?? "",
    `解析结果缓存尚未实现，无法处理 ${request.type}（P2 落地）`,
