...
const M = [
  ...
  P('gasSpend','支出','气体支出','Vespene Used (all)',
    (p,i)=>p.series.vUsedEco[i]+p.series.vUsedTech[i]+p.series.vUsedArmy[i]),

  P('armyValue','军事','军队价值','Army Value (min + gas)',
  ...
  P('lostTotal','战损','总损失','All Lost (minerals)',
  ...
  P('killTotal','战果','总战果','All Killed (minerals)',
...
function renderTimeline(){
  ...
  merged.forEach(e => {
    ...
    d.title = `${e.kind === 'battle' ? '交战' : '工人损失'} @ ${mmss(e.t)}（${e.who === 'a' ? a.name : b.name} 侧）`;
    lay.appendChild(d);
  });
  ...
  $('#tllegend').innerHTML = [
    ...
    `<span><i style="background:${COL.a};width:2px;height:10px;border-radius:1px;opacity:.6"></i>军队价值单步跌幅超峰值 5%（交战）</span>`,
    `<span style="color:var(--ink4)">中线上下两片 = 双方军队价值差的镜像包络</span>`,
  ].join('');
}
...
function renderSide(){
  ...
  host.appendChild(el('h3', null, '指标（官方 overlay 口径）'));
  GROUPS.forEach(g => {
...
async function ovProbe(timeoutMs = 1500){
  ovSet('wait', `正在探测 127.0.0.1:${OV_PORT} …`);
  ...
  try {
    ...
    ovSet('ok', `已连接：${info.name || 'overlay-agent'} v${info.version || '?'}（外置置顶窗口）`);
    return true;
  } catch (err) {
    const denied = err?.name === 'NotAllowedError';
    ovSet('bad', denied
      ? '本地网络访问被拒绝 —— 请在地址栏权限提示里允许访问本地网络后重试'
      : `未检测到本地悬浮组件（${OV_PORT} 端口无响应）`);
    return false;
...
async function openPip(){
  ...
  if (!('documentPictureInPicture' in window)) { alert('当前浏览器不支持 Document 画中画。'); return; }
  try {
    ...
    renderPip();
    ovSet('wait', '已降级为浏览器内 Document 画中画（非置顶透明窗口）');
  } catch (err) {
    pipWin = null;
