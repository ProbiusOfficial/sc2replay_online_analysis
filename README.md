...
一个基于浏览器端的《星际争霸 II》录像在线解析工具。在仓库根目录启动任意静态 HTTP 服务后访问 `index.html`，将本地 `.SC2Replay` 拖入页面（或点击选择），即可在浏览器内完成解析与展示，无需后端。
...
| 路径 | 说明 |
|------|------|
| `index.html` | 页面骨架、Pyodide / Chart.js CDN、`js/app.js` 模块入口 |
| `css/app.css` | 全局样式 |
| `js/app.js` | 事件绑定与初始化 |
