...
*新增勾选项 “显示初始化事件”（默认不勾选）,优化显示体验:
默认会过滤以下“初始化事件”，勾选后全部显示：
开局 0 秒的 RewardDance* 升级事件。
开局 0 秒的 Beacon* 相关单位生产。
补给占用 ≤ 11 的所有单位生产事件。
开局 2 秒内 的所有建筑建造事件。
虫族额外规则：Z 开局 1 秒内 的王虫（Overlord*）生产事件。

*前端时间轴渲染调整:
时间轴排序与展示逻辑统一基于修正后的 start_time / finish_time。
初始化事件过滤在排序和渲染前完成，通过 isInitEvent(item, race) 判断，并根据 “显示初始化事件” 开关决定是否保留。
```
