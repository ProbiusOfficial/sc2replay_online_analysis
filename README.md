...
- **沙盘模拟**：在抽象沙盘上重演对局 —— **斜视角**（45° 菱形等距投影 + 基地椭圆平台，可切回平面）、**缩放/平移**（滚轮以光标为焦点缩放 1–10×、拖拽平移可到任意边缘区域、双击画布/「适配」按钮复位，右缘显示倍率；单位/建筑图标按 ≈zoom^0.75 随缩放生长，均对标 starcraft2.ai 实测手感）、**左上/右上面板**（玩家名 + 单位/农民/建筑实时计数 + 编成 chips + 生产条 + 人口/军队）、**底部资源条**（双方矿/气 + 采集率 + 人口）、单位用**真实游戏图标**渲染（素材为无 alpha 的黑底 webp，加载时按亮度→alpha 抠成纯图标；图标外圈/底色 = 归属方）、军队走位（稀疏采样点间插值）、矿线 / 气泉 / Xel'Naga 塔、交战涟漪（精确死亡坐标）。与全局时间轴同一条游标，可 1~16× 播放。**整页让位的大画布** + **⛶ 全屏**。**不需要地图文件**。这是上帝视角的抽象沙盘，不是拟真小地图（录像里没有地形 / 迷雾 / 血量）。
