...
来源：原 `js/parse_script.js` 里的 `export const PARSE_SCRIPT` 模板串，逐字节剥壳而来
（路线 C 之前站点在浏览器里用 Pyodide 执行的正是这段 Python）。Pyodide 已下线，
这段代码**不再被站点加载**，唯一用途是 `scripts/freeze-baseline.py` ——
重建 `tests/baseline/*.json` 那份字段级对拍基准。

因此：
- 不要为了「让新实现过测」而改它。改它 = 换基准 = 自证。
- 不要把它挪回 `js/`。它在 `js/` 里会被打成站点资源，也会让人误以为还在跑。
- `scripts/verify-replay-data.mjs` 的对拍基准是 `tests/baseline/*.json`，与本文件无关。

依赖版本已被 `scripts/freeze-baseline.py` 固定为 sc2reader 1.9.0 / spawningtool 3.0.0。
"""
